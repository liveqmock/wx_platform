				...
				List<MessageBean> messageBeanList  = (List<MessageBean>)request.getAttribute("messageBeanList");
			      ...
			      %>    
			          ...
			          <tr>
						...
						%>
						<button type="button" class="btn btn-default" style="padding: 0 12px" title="编辑" onclick="edit_service(<%=messageBean.getId()%>)"><span class="icon-edit"></span></button>
						<%} else { %>
						<button type="button" class="btn btn-default disabled" disabled="true" style="padding: 0 12px" title="编辑" onclick="edit_service(<%=messageBean.getId()%>)"><span class="icon-edit"></span></button>
						<%} %>
						</td>
			          </tr>
