...
<body class="page_appmsg_edit"  >
	...
	<%  
		ResearchbasicBean researchbasicBean = (ResearchbasicBean)request.getAttribute("researchbasicBean"); 
	%>
	<form id="researchbasic">
							...
							<div id="appmsgItem1" data-fileid="" data-id="1"
								...
								<h4 class="appmsg_title">
									<a onclick="return false;" href="javascript:void(0);"
										target="_blank"><%=researchbasicBean.getTitle() %></a>
								</h4>
								<div class="appmsg_info">
									<em class="appmsg_date"><%=researchbasicBean.getCreatetime() %></em>
								</div>
								<div class="appmsg_thumb_wrp">
									<img class="js_appmsg_thumb appmsg_thumb" src="<%=researchbasicBean.getImg() %>">
					...
					<div class="appmsg_editor" style="margin-top: 0px;">
						<div class="inner">
							<div class="appmsg_edit_item">
								...
								<span >
									<input type="text" class="frm_input" name="researchbasicViewModel.keywords" required="required" value="<%=researchbasicBean.getKeywords() %>">
								</span>
							</div>
							<div class="appmsg_edit_item">
								<label for="" class="frm_label">
								...
								<span >
									<input type="text" class="frm_input js_title" name="researchbasicViewModel.title" required="required" value="<%=researchbasicBean.getTitle() %>">
								</span>
							</div>							
							<div class="appmsg_edit_item">
								<label for="" class="frm_label">
								...
								<div class="frm_input_box">
									...
									<p class="js_cover upload_preview" style="display: block;">
										<img src="<%=researchbasicBean.getImg() %>" id="upload_preview_img" style="margin-bottom: 10px;">
							...
							<div class="appmsg_edit_item">
								...
								<span  >
								<input id="datetimepicker_start" type="text" readonly="readonly" class="datetimepicker" name="researchbasicViewModel.begintime" required="required" value="<%=researchbasicBean.getBegintime().substring(0,19)%>">
									至
								<input id="datetimepicker_end" type="text" readonly="readonly" class="datetimepicker" name="researchbasicViewModel.endtime" required="required" value="<%=researchbasicBean.getEndtime().substring(0,19)%>">
								</span>
							...
							<div class="js_desc_area dn appmsg_edit_item">
								...
								<textarea style="height: 45px" maxlength="120" class="js_desc frm_textarea textarea" required="required" name="researchbasicViewModel.descs"><%=researchbasicBean.getDescs() %></textarea>
							</div>
							
							<div class="js_desc_area dn appmsg_edit_item">
								<label for="" class="frm_label">调研结束语</label>
								<textarea style="height: 45px" maxlength="120" class="js_desc frm_textarea textarea" required="required" name="researchbasicViewModel.enddescs"><%=researchbasicBean.getEnddescs() %></textarea>
							</div>
						</div>
					</div>
				</div>
		...
		<input type="hidden" name="researchbasicViewModel.id" value="<%=researchbasicBean.getId() %>">		
</form>
</body>
<script>
	...
	$(document).ready(function(){
		...
		//图像
		$('.js_appmsg_thumb').attr('src','<%=researchbasicBean.getImg() %>');
		$('.js_appmsg_thumb').css('display','block');
		$('.frm_input_hidden').val('<%=researchbasicBean.getImg() %>');
		//摘要
    ...
    function addSave() {
		...
		if(!cansv){
			...
			$.ajax({
				...
				type : 'POST',
				data : params,
				url : 'researchbasic!updateResearchbasic.action', //请求的action路径
