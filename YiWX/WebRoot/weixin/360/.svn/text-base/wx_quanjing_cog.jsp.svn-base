...
.fontRed{
	...
	<body class="page_advanced_reply">
		...
		<div class="main_bd">
			<form id="quanjing" style="margin: 0px;">
									...
									<div class="sub_title_bar no_extra">
										<h3>
											...
											<span class="reply_title_tips"> <a
												>关键词相同时,返回给用户最新创建的全景!</a>
											</span>
											<a href="<%=path%>/quanjing!add.action" >
											<span  class="btn"  title="添加全景图" ><i class="icon-plus"></i>
											添加全景图</span>
											</a>	
											<!-- 
											...
											查询</span>
											 ...
											 -->
											<input name="quanjingViewModel.title" type="hidden" placeholder="请输入全景图标题" style="float: right;margin-left:5px;margin-top: 3px"/>
	...
	function requeryCogPage(page){
		...
		$.ajax({
			...
			type : 'POST',
			data : params,
			url : 'quanjing!requeryCogPage.action', //请求的action路径
	...
	function requeryCogResult(){
		...
		$.ajax({
			...
			type : 'POST',
			data : params,
			url : 'quanjing!requeryCogResult.action', //请求的action路径
			error : function() {//请求失败处理函数);
				alert('查询失败');
			},
			success : function(data) { //请求成功后处理函数
	...
	$(document).ready(function(){
  	...
  	function del_service(id) {
  		...
  		if(confirm('是否确定删除此全景？此操作不可以恢复！')) { 
			$.ajax({
				async : false,
				cache : false,
				type : 'POST',
				data : params,
				url : 'quanjing!deleteQuanJing.action', //请求的action路径
				error : function() {//请求失败处理函数);
					alert('服务器访问失败');
				},
				success : function(data) { //请求成功后处理函数
  	...
  	function edit_service(id) {
  		loadContent('quanjing!edit.action?id='+id);
  	}
  	
  	function forUsed(id) {
  		var left = ($(window.parent.parent).width() - 450) / 2;
		window.open("<%=path %>/quanjing!yulan.action?serviceid="+id, "我的微相册", "height=650,width=450,top=0,left=" + left + ",toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no");
  	}
