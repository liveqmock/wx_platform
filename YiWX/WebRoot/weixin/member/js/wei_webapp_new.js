...
var APP = (function(window){ //MVC结构
	var _cfg = {},
		...
		_utils = {
			...
			fixBtnHook: function(on) //旧浏览器底部大按钮的fixed
			{
				if (_q('#footBtn') || _q('.footFix'))
				{
					if (on == undefined) on = true;
					if ( _isOldIOS || _isOldAndroid22 || _isMeizu || !_testFixedSupport() )
					{
						if ( !_isOldIOS )
						{
							if (on)
								_hook1TO = window.setInterval(_fixFootBtnForOldIOS, 500);
							else
								window.clearInterval(_hook1TO);
						}else
						{
							if (on){
								_hook1TO = window.setTimeout(_fixFootBtnForOldIOS, 200);
								...
								window.removeEventListener('scroll',_fixFootBtnForOldIOS);
								window.removeEventListener('resize',_fixFootBtnForOldIOS);
								window.removeEventListener('touchmove',_fixFootBtnForOldIOS);
								window.removeEventListener('touchend',_fixFootBtnForOldIOS);
							}
						}
					}					
				}
		...
		_e = {
			...
			commonAjaxClick: function(e)
			{
				...
				*/
				if ( typeof hObj.act === 'string' && /^(https?|weixin)\:\/{2}/.test(hObj.act) ){
...
var TemplateParser = (function(){
	...
	p.prototype = {
		...
		doHide: function(p_tmpl){
			var rtn = p_tmpl;
			rtn = rtn.replace(/data-hidden-when-lost=\"undefined\"/g, 'hidden');
			rtn = rtn.replace(/data-hidden-when-lost=\"null\"/g, 'hidden');
			rtn = rtn.replace(/data-hidden-when-lost=\"\{\#{1}[^\#]+\#{1}\}(\s|\&nbsp\;)*\"/g, 'hidden');
			rtn = rtn.replace(/data-hidden-when-lost=\"\_field\.([^\_]+)\_\"/g, 'hidden');
