...
.fontRed{
	...
	<body class="page_advanced_reply">
		...
		%>
		<div class="main_bd">
			<form id="photo" style="margin: 0px;">
			<input  type="hidden" id="toPage" name="pageBean.currentPage" value="1"/>
			<input  type="hidden" name="liuyingInfoBean.service_id" value="<%=service_id%>"/>
		...
		<form id="form_del">
		 	<input id="info_id" type="hidden" name="liuyingInfoBean.id" value="0">
		</form>	
	</body>
	...
	function requeryCogPage(page){
		...
		$.ajax({
			...
			type : 'POST',
			data : params,
			url : 'liuying!requeryCogPage.action', //请求的action路径
			...
			success : function(data) { //请求成功后处理函数
				...
				//显示人数
				$('#liuying_count').html( josnStr["totalRecord"]);	
	...
	function requeryCogResult(){
		...
		$.ajax({
			...
			type : 'POST',
			data : params,
			url : 'liuying!requeryCogResult.action', //请求的action路径
			error : function() {//请求失败处理函数);
				alert('替换数据失败');
			},
			success : function(data) { //请求成功后处理函数
	...
	$(document).ready(function(){
  	...
  	function del(id) {
  		...
  		if(confirm('是否确定删除此留言？此操作不可以恢复！')) { 
			$.ajax({
				async : false,
				cache : false,
				type : 'POST',
				data : params,
				url : 'liuying!deleteOneInfo.action', //请求的action路径
				error : function() {//请求失败处理函数);
					alert('服务器访问失败');
				},
				success : function(data) { //请求成功后处理函数
