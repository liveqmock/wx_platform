...
.fontRed{
	...
	<body class="page_advanced_reply">
		...
		<div class="main_bd">
			...
			<input  type="hidden" name="photoViewModel.id" value="<%=serviceid %>"/>
				...
				<div class="bd">
					<div class="sub_title_bar no_extra">
						<h3>
							...
							<a href="<%=path%>/photo!add.action?serviceid=<%=serviceid %>" >
							...
							查询</span>
							 ...
							 -->
							<input name="photoViewModel.title" type="hidden" value="">
							<!-- 
	...
	function requeryCogPage(page){
		...
		$.ajax({
			...
			type : 'POST',
			data : params,
			url : 'photo!requeryCogPage.action', //请求的action路径
	...
	function requeryCogResult(){
		...
		$.ajax({
			...
			type : 'POST',
			data : params,
			url : 'photo!requeryCogResult.action', //请求的action路径
			error : function() {//请求失败处理函数);
				alert('替换数据失败');
			},
			success : function(data) { //请求成功后处理函数
	...
	$(document).ready(function(){
  	...
  	function edit_service(id) {
  		loadContent('photo!edit.action?&serviceid=<%=serviceid%>&id='+id);
  	}
