...
<body class="page_appmsg_edit"  >
	...
	<%  
		MessagebasicBean messagebasicBean = (MessagebasicBean)request.getAttribute("messagebasicBean"); 
	%>
	<form id="messagebasic">
							...
							<div id="appmsgItem1" data-fileid="" data-id="1"
								...
								<h4 class="appmsg_title">
									<a onclick="return false;" href="javascript:void(0);"
										target="_blank"><%=messagebasicBean.getTitle() %></a>
								</h4>
								<div class="appmsg_info">
									<em class="appmsg_date"><%=messagebasicBean.getCreatetime() %></em>
								</div>
								<div class="appmsg_thumb_wrp">
									<img class="js_appmsg_thumb appmsg_thumb" src="<%=messagebasicBean.getImg() %>">
					...
					<div class="appmsg_editor" style="margin-top: 0px;">
						<div class="inner">
							<div class="appmsg_edit_item">
								...
								<span >
									<input type="text" class="frm_input" name="messagebasicViewModel.keywords" required="required" value="<%=messagebasicBean.getKeywords() %>">
								</span>
							</div>
							<div class="appmsg_edit_item">
								<label for="" class="frm_label">
								...
								<span >
									<input type="text" class="frm_input js_title" name="messagebasicViewModel.title" required="required" value="<%=messagebasicBean.getTitle() %>">
								</span>
							</div>							
							<div class="appmsg_edit_item">
								<label for="" class="frm_label">
								...
								<div class="frm_input_box">
									...
									<p class="js_cover upload_preview" style="display: block;">
										<img src="<%=messagebasicBean.getImg() %>" id="upload_preview_img" style="margin-bottom: 10px;">
										<input  type="hidden" class="frm_input js_title frm_input_hidden" name="messagebasicViewModel.img">
							...
							<div class="js_desc_area dn appmsg_edit_item">
								...
								<textarea style="height: 45px" maxlength="120" class="js_desc frm_textarea textarea" required="required" name="messagebasicViewModel.descs"><%=messagebasicBean.getDescs() %></textarea>
							</div>
						</div>
					</div>
				</div>
		...
		<input type="hidden" name="messagebasicViewModel.id" value="<%=messagebasicBean.getId() %>">		
</form>
</body>
<script>
	var  urlNum = 0; 
	$(document).ready(function(){
		...
		//图像
		$('.js_appmsg_thumb').attr('src','<%=messagebasicBean.getImg() %>');
		$('.js_appmsg_thumb').css('display','block');
		$('.frm_input_hidden').val('<%=messagebasicBean.getImg() %>');
		//摘要
    ...
    function addSave() {
    	var cansv= true;
		$('#messagebasic').find('input[type="text"],select,textarea').filter('[required="required"]').each(function(){
			if($.trim($(this).val())==''){
				cansv = false;
		...
		if(!cansv){
			...
			$.ajax({
				...
				type : 'POST',
				data : params,
				url : 'messagebasic!updateMessagebasic.action', //请求的action路径
