...
public class GuaguakaAction extends ActionSupport {
		...
		try {
			...
			if(serviceid!=null){
				...
				guaguakaBean.setId(Long.parseLong(serviceid));
 				guaguakaBean = guaguakaService.selectServiceByID(guaguakaBean);
	...
	private boolean verifyWin(GuaguakaBean guaguakaBean) {
		boolean temp=false;
		GuaguakaService guaguakaService = (GuaguakaService) ApplicationContextFactory.getBean(Constat.GUAGUAKASERVICE);
		GuaguakaInfoBean guaguakaInfoBean = guaguakaService.selectIsWin(guaguakaBean);
	...
	public String requeryInfo() throws Exception {
		GuaguakaService guaguakaService = (GuaguakaService) ApplicationContextFactory.getBean(Constat.GUAGUAKASERVICE);
		GuaguakaInfoBean guaguakaInfoBean = guaguakaService.selectOneInfo(guaguakaBean);
		String result = null;
		if(guaguakaInfoBean==null){
			guaguakaService.insertOneInfo(guaguakaBean);
		}else{
			guaguakaBean = guaguakaService.selectServiceByID(guaguakaBean);
			//1判断总数
	...
	public String requeryWinInfo() throws Exception {
		...
		//计算中奖率
 		...
 		if(temp<guaguakaBean.getProbability()){
 			//2是否还有奖品
 			List<GuaguakaInfoBean> guaguakaInfoBeanList = guaguakaService.selectAllOfWin(guaguakaBean);
 			...
 			if(prize_surplus>0){
 				...
 				guaguakaService.updateInfoForWin(guaguakaInfoBean);
	...
	public String updateInfo() throws Exception{
		...
		guaguakaService.updateInfoForTime(guaguakaInfoBean);
		inputStream = new ByteArrayInputStream(result.getBytes("UTF-8"));
		return SUCCESS;
	}
	...
	public String list(){
		...
		List<GuaguakaBean> guaguakaBeanList = guaguakaService.selectAllByAppID(guaguakaBean);
	...
	public String addSave() throws Exception{
		...
		if(guaguakaBean.getKeywords().length()==0||guaguakaBean.getKeywords().length()>30){
			...
			//校验成功,录入数据库
			GuaguakaService guaguakaService = (GuaguakaService) ApplicationContextFactory.getBean(Constat.GUAGUAKASERVICE);
			if(guaguakaService.insertOneService(guaguakaBean)>0){
				// 插入活动成功后，是否插入 素材库或者关键字中，如果不的话，查询关键字表的时候如何查询 刘虹辰TODO
				//更新中间表
				ActionLinkManager.updateNewActionLink(AnythingToActionLink.getActionLinkByGuaguaka(guaguakaBean));
				result="success";
			}
		}
	...
	public String editSave() throws Exception{
		...
		if(guaguakaBean.getKeywords().length()==0||guaguakaBean.getKeywords().length()>30){
			...
			//校验成功,修改数据
			GuaguakaService guaguakaService = (GuaguakaService) ApplicationContextFactory.getBean(Constat.GUAGUAKASERVICE);
			if(guaguakaService.updateOneService(guaguakaBean)>0){
				// 插入活动成功后，是否插入 素材库或者关键字中，如果不的话，查询关键字表的时候如何查询 刘虹辰TODO
				ActionLinkManager.updateNewActionLink(AnythingToActionLink.getActionLinkByGuaguaka(guaguakaBean));
				result="success";
			}
	...
	public String del() throws Exception {
		...
		String result = null;
		GuaguakaService guaguakaService = (GuaguakaService) ApplicationContextFactory.getBean(Constat.GUAGUAKASERVICE);
		if(guaguakaService.delOneService(guaguakaBean)>0){
			guaguakaService.delInfosByServiceId(guaguakaBean);
			ActionLinkManager.deleteActionLink(AnythingToActionLink.getActionLinkByGuaguaka(guaguakaBean));
			result="success";
		}
		inputStream = new ByteArrayInputStream(result.getBytes("UTF-8"));
		return SUCCESS;
	...
	public String cogInfo(){
		...
		if(id!=null){
			guaguakaBean.setId(Long.parseLong(id));
			GuaguakaService guaguakaService = (GuaguakaService) ApplicationContextFactory.getBean(Constat.GUAGUAKASERVICE);
			List<GuaguakaInfoBean> guaguakaInfoBeanList = guaguakaService.selectAllInfosByServiceId(guaguakaBean);
	...
	public String requeryCogPage() {
		GuaguakaService guaguakaService = (GuaguakaService) ApplicationContextFactory.getBean(Constat.GUAGUAKASERVICE);
		int totalRecord = guaguakaService.selectCountInfosByPage(guaguakaInfoBean);
	...
	public String requeryCogResult(){
		...
		List<GuaguakaInfoBean> guaguakaInfoBeanList = guaguakaService.selectInfosByPage(guaguakaInfoBean);
	...
	public String subPassword() throws Exception{
		GuaguakaService guaguakaService = (GuaguakaService) ApplicationContextFactory.getBean(Constat.GUAGUAKASERVICE);
		String result = "";
		guaguakaInfoBean = guaguakaService.selectOneInfoById(guaguakaInfoBean);
		if(guaguakaInfoBean.getType()==2){
			result = "error";
		}else{
			if(guaguakaService.updateInfoForUsed(guaguakaInfoBean)){
				result="success";
			}
		}
		inputStream = new ByteArrayInputStream(result.getBytes("UTF-8"));
	...
	public String subContact() throws Exception{
		GuaguakaService guaguakaService = (GuaguakaService) ApplicationContextFactory.getBean(Constat.GUAGUAKASERVICE);
		String result = null;
		if(guaguakaService.updateInfoForContact(guaguakaInfoBean)){
			result="success";
		}
		inputStream = new ByteArrayInputStream(result.getBytes("UTF-8"));
		return SUCCESS;
